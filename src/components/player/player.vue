<template>
  <div class="player" v-show="playlist.length>0">
    ...
    <div class="mini-player" v-show="!fullScreen" @click="open">
      ...
      <div class="control">
        <i @click.stop="togglePlaying" class="icon-playlist"></i>
      </div>
    </div>
    </transition>
    <audio @error="error" @canplay="ready" ref="audio" :src="currentSong.url" @timeupdate="updateTime"></audio>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      leave(el, done) {
        ...
        const {x, y, scale} = this._getPosAndScale()
        this.$refs['cd-wrapper'].style[transform] = `translate3d(${x}px,${y}px,0) scale(${scale})`
        this.$refs['cd-wrapper'].addEventListener('transitionend', done)
      },
      afterLeave() {
        this.$refs['cd-wrapper'].style.animation = ''
